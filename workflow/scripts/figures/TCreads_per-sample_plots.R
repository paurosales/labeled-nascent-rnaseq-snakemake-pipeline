...
enhancers <- read.table(enhancersTSV, header=TRUE, sep="\t")
...
p_freq <- ggplot(collapsedCountsExtra, aes(TcReadRange)) + # p values for genes with mean normalized count larger than 1
...
p_freq <- ggplot(collapsedCountsExtra[collapsedCountsExtra$TcReadRange!="0",], aes(TcReadRange)) + # p values for genes with mean normalized count larger than 1
        geom_bar(fill = palette[2]) +
